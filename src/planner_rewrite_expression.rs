...
pub(crate) fn rewrite_conditional(expr: Expr, schemas: &Schemas) -> Result<Expr> {
...
/// incompatible.
pub(crate) fn rewrite_expr(expr: Expr, schemas: &Schemas) -> Result<Expr> {
...
mod test {
    use super::*;
    use crate::datafusion_util::AsExpr;
    use crate::provider::InfluxFieldType;
    use crate::test_utils::{to_arrow_schema, MockSchemaBuilder};
    use datafusion::common::ToDFSchema;
    use datafusion::logical_expr::lit_timestamp_nano;
    use std::sync::Arc;

    fn new_schemas() -> Schemas {
        let iox_schema = MockSchemaBuilder::new()
            .timestamp()
            .tag("tag0")
            .tag("tag1")
            ...
            .build();
        let df_schema = to_arrow_schema(&iox_schema).to_dfschema().unwrap();
        Schemas {
            df_schema: Arc::new(df_schema),
            iox_schema: Arc::new(iox_schema),
        }
    }
